    ...
    <el-card shadow="never" style="border:none">
        <div class="mb-4 flex lg:flex-row md:flex-row gap-4 flex-col justify-between items-center">
            <div class="w-full lg:w-1/2 md:w-1/2">
                <h1 class="text-xl font-semibold">Kelola Pengajuan</h1>
            </div>
        </div>
...
const customColumns = ref([
    { prop: "nama_instansi", label: "Instansi", width: 'auto' },
    { prop: "nama_bantuan_sosial", label: "Nama Bantuan Sosial", width: 120 },
    { prop: "periode_pendaftaran_id", label: "Periode", width: 150 },
    { prop: "status", label: "Status", width: '120px' },
    { prop: "created_at", label: "Dibuat pada", width: 120 },
]);
...
const openPotentialBeneficiary = (data: any,) => {
    router.push({ name: 'manage-potential-beneficiary', params: { id: router.currentRoute.value.params.id, submission_id: data.id } })
}
...
const fetchDataWithCurrentParams = () => {
    ...
    store.fetchData(currentPage.value, currentPerPage.value, searchQuery.value, '', '', String(route.params.id), statusFilter.value);
};
