<template>
    <el-card shadow="never" class="mb-4" style="border:none">
        <el-descriptions title="Data Kriteria" :column="2" border>
            ...
            <el-descriptions-item label="Bukti Pendukung">{{ criteria?.data.perlu_bukti_pendukung ? 'Ya' :
                'Tidak'}}</el-descriptions-item>
        </el-descriptions>
    </el-card>
    <el-card shadow="never" style="border:none">
        ...
        <div class="my-4 flex lg:flex-row md:flex-row gap-4 justify-between items-center">
            ...
            <div class="flex lg:flex-row md:flex-row items-center justify-end gap-5 w-1/2">
                <el-input placeholder="Cari Sub Kriteria ..." v-model="searchQuery" prefix-icon="el-icon-search"
                    class="lg:w-1/2 md:w-1/2 w-full" clearable />
            </div>
        </div>

        <div>
            <Datatable :selection="true" :items="sub_criterias" :pagination="pagination" :loading="loading"
            ...
            <crud-modal v-model:visible="crudModalVisible" :mode="crudModalMode" :data="crudModalData"
                ...
                name="Sub Kriteria Bantuan" :create-service="store.createData" :update-service="store.updateData"
            ...
            <Weighting v-model:visible="dialogWeightingVisible" :data="sub_criterias"
                :create-service="pairWise.createData" :delete-service="pairWise.deleteByBantuan"
                :update-service="store.updateBobot" tipe="sub_kriteria" />
        </div>
    </el-card>
</template>
